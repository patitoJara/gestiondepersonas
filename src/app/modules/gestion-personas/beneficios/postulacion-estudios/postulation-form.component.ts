...
export class PostulationFormComponent {
  ...
  // 🔥 Evita pestañeos visuales mientras carga el workflow inicial
  hideStepperBySystemClock = true;
  ...
  async ngOnInit() {
    ...
    try {
      ...
      console.error('❌ ERROR INIT POSTULATION FORM:', e);

      this.showError('Error cargando formulario de postulación');
    } finally {
  ...
  async saveStep4AcademicBackground() {
    ...
    try {
      ...
      const payload: any = {
        ...
        hadPreviousBenefit: Boolean(this.form.value.beneficiado),
      };
  ...
  async submitPostulation() {
    ...
    try {
      ...
      // 1) Traer el summary completo ANTES de enviar.
      // Este summary contiene familiares, gastos, salud, vivienda y documentos.
      latestSummary = await firstValueFrom(
      ...
      // 3) Mantener el summary completo y reemplazar solo la cabecera de postulación.
      // IMPORTANTE: no usar la respuesta del submit como summary completo, porque no trae arrays.
      // Si se usa solo response, visualmente se pierden familiares/gastos/documentos en Step 11.
      this.summary = {
        ...(latestSummary || this.summary || {}),
      ...
      // 4) Refresco no bloqueante. Si falla, NO limpia la pantalla.
      setTimeout(async () => {
        try {
  ...
  generarComprobanteHTML(): string {
    ...
    const beneficiarioNombre =
      this.tipoPostulante === 'afiliado'
        ? `${this.form.value.nombre} ${this.form.value.apellido}`
        : this.postulanteSeleccionado?.nombre || 'No informado';
    ...
    const grupoFamiliarHTML = this.grupoFamiliar
      .map(
        (f: any, i: number) => `
        <tr>
          <td>${f.nombre}</td>
          <td>${this.getParentTypeName(f.parentTypeId)}</td>
          <td>${i === 0 ? this.form.value.calidadContractual : 'PASIVO'}</td>
        </tr>
      `,
      )
      .join('');

    const ingresosHTML = this.ingresosFamiliares
      .map(
        (i: any) => `
        <tr>
          <td>${this.getNombreIngreso(i.familiarId)}</td>
          <td>$${Number(i.monto || 0).toLocaleString('es-CL')}</td>
        </tr>
      `,
      )
      .join('');

    const saludHTML = this.salud
      .filter((s: any) => s.patologia || s.gasto)
      .map(
        (s: any) => `
        <tr>
          <td>${this.getNombreFamiliar(s.familiarId)}</td>
          <td>${s.patologia}</td>
          <td>$${Number(s.gasto || 0).toLocaleString('es-CL')}</td>
        </tr>
      `,
      )
      .join('');

    const documentosHTML = this.documentosObligatorios
      .map(
        (doc: any) => `
        <li>✔ ${doc.label}</li>
      `,
    ...
    return `
  <div style="font-family: Arial, sans-serif; font-size:12px; padding:25px; max-width:900px; margin:auto; color:#222;">

    <!-- HEADER -->
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
      <img src="${logoGob}" style="height:60px;" />
      <img src="${logoBienestar}" style="height:60px;" />
    </div>

    <h2 style="text-align:center; color:#1565c0; margin-bottom:5px;">
    ...
    <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:15px;">
      <div><strong>Fecha:</strong> ${new Date().toLocaleString('es-CL')}</div>
      <div><strong>Código:</strong> ${this.codigoPostulacion}</div>
    </div>

    <hr/>

    <!-- DATOS AFILIADO -->
    <h3 style="color:#1565c0;">Datos del afiliado</h3>

    <table style="width:100%; border-collapse:collapse;">
      <tr>
        <td><strong>Nombre:</strong> ${this.form.value.nombre} ${this.form.value.apellido}</td>
        <td><strong>RUT:</strong> ${this.form.value.rut}</td>
      </tr>
      <tr>
        <td><strong>Teléfono:</strong> ${this.form.value.telefono}</td>
        <td><strong>Email:</strong> ${this.form.value.email}</td>
      </tr>
      <tr>
        <td><strong>Planta:</strong> ${this.form.value.planta}</td>
        <td><strong>Grado:</strong> ${this.form.value.grado}</td>
      </tr>
      <tr>
        <td><strong>Calidad contractual:</strong> ${this.form.value.calidadContractual}</td>
        <td><strong>Tipo afiliado:</strong> ${this.form.value.tipoAfiliado}</td>
      </tr>
      <tr>
        <td colspan="2"><strong>Hogar monoparental:</strong> ${
          this.form.value.hogarMonoparental ? 'Sí' : 'No'
        }</td>
      </tr>
    </table>
    ...
    <!-- BENEFICIARIO -->
    <h3 style="color:#2e7d32;">Beneficiario</h3>

    <div><strong>Nombre:</strong> ${beneficiarioNombre}</div>
    <div><strong>Tipo:</strong> ${beneficiarioTipo}</div>

    <hr/>

    <!-- GRUPO FAMILIAR -->
    <h3 style="color:#6a1b9a;">Grupo familiar</h3>
    ...
    <table style="width:100%; border-collapse:collapse; border:1px solid #ddd;">
      <thead style="background:#f5f5f5;">
        <tr>
          ...
          <th style="padding:6px; border:1px solid #ddd;">Condición</th>
        </tr>
      </thead>
      <tbody>
        ${grupoFamiliarHTML}
    ...
    <!-- ACADÉMICO -->
    <h3 style="color:#1565c0;">Antecedentes académicos</h3>

    <div><strong>Institución:</strong> ${this.academico.institucion}</div>
    <div><strong>Carrera:</strong> ${this.academico.carrera}</div>
    <div><strong>Nivel:</strong> ${this.getStudyName(this.academico.studyId)}</div>
    <div><strong>Semestre actual:</strong> ${this.academico.semestre}</div>
    <div><strong>Duración:</strong> ${this.academico.duracion} semestres</div>
    <div><strong>Estudia en la región:</strong> ${
      this.academico.region === 'si' ? 'Sí' : 'No'
    }</div>

    <hr/>

    <!-- INGRESOS -->
    <h3 style="color:#2e7d32;">Ingresos familiares</h3>

    <table style="width:100%; border-collapse:collapse; border:1px solid #ddd;">
      <thead style="background:#f5f5f5;">
        <tr>
          <th style="padding:6px; border:1px solid #ddd;">Integrante</th>
          <th style="padding:6px; border:1px solid #ddd;">Monto</th>
        </tr>
      </thead>
      <tbody>
        ${ingresosHTML}
    ...
    <div style="margin-top:8px;">
      ...
      $${Number(this.totalIngresosFamiliares || 0).toLocaleString('es-CL')}
    </div>

    <hr/>

    <!-- GASTOS -->
    <h3 style="color:#ef6c00;">Gastos familiares</h3>

    <div><strong>Gastos básicos:</strong> $${Number(
      this.totalGastosBasicos || 0,
    ).toLocaleString('es-CL')}</div>

    <div><strong>Educación:</strong> $${Number(
      this.totalGastosEducacion || 0,
    ).toLocaleString('es-CL')}</div>

    <div><strong>Otros gastos:</strong> $${Number(
      this.totalOtrosGastos || 0,
    ).toLocaleString('es-CL')}</div>

    <div style="margin-top:8px;">
      <strong>Total gastos:</strong>
      $${Number(this.totalGastos || 0).toLocaleString('es-CL')}
    </div>

    <hr/>

    <!-- VIVIENDA -->
    <h3 style="color:#1565c0;">Vivienda</h3>

    <div><strong>Tipo vivienda:</strong> ${
      this.getHousingName(this.form.value.tipoVivienda) || 'No informado'
    }</div>

    <div><strong>Tipo propiedad:</strong> ${
      this.getPropertyName(this.form.value.tipoPropiedad) || 'No informado'
    }</div>

    <hr/>

    <!-- SALUD -->
    <h3 style="color:#c62828;">Salud</h3>
    ...
    ${
      ...
      <table style="width:100%; border-collapse:collapse; border:1px solid #ddd;">
        <thead style="background:#f5f5f5;">
          <tr>
            <th style="padding:6px; border:1px solid #ddd;">Integrante</th>
            <th style="padding:6px; border:1px solid #ddd;">Patología</th>
            <th style="padding:6px; border:1px solid #ddd;">Gasto</th>
          </tr>
        </thead>
        <tbody>
          ${saludHTML}
    ...
    `
        : '<div>Sin antecedentes de salud registrados.</div>'
    }

    <div style="margin-top:8px;">
      <strong>Total salud:</strong>
      $${Number(this.totalSalud || 0).toLocaleString('es-CL')}
    </div>

    <hr/>

    <!-- DOCUMENTOS -->
    <h3 style="color:#6a1b9a;">Documentos adjuntos</h3>
    ...
    <!-- ESTADO FINAL -->
    <div style="margin-top:25px; text-align:center; color:#2e7d32; font-weight:bold; font-size:16px;">
      ✔ POSTULACIÓN RECIBIDA EXITOSAMENTE
    </div>

    <div style="text-align:center; margin-top:8px; color:#555;">
      Bienestar Servicio de Salud Magallanes
    </div>
  ...
  private getStep1Errors(): string[] {
    ...
    if (!form.calidadContractual) {
      errores.push('Situación Contractual');
    }
  ...
  private restoreFullStateFromSummary(summary: any): void {
    ...
    this.form.patchValue({
      hogarMonoparental: postulation?.isSingleParentHome === true,
    });
